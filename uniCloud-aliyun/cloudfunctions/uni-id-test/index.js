...
exports.main = async (event, context) => {
  ...
  let noCheckAction = ['register', 'checkToken', 'encryptPwd', 'login', 'loginByWeixin', 'loginByQQ', 'sendSmsCode',
    'verifyCode', 'loginBySms', 'loginByEmail', 'code2SessionWeixin', 'code2SessionAlipay'
  ]
  ...
  switch (event.action) {
    ...
    case 'sendSmsCode': {
      const {
        mobile,
    //  实际项目中code应由云端生成，这里为了方便演示由客户端上传
        type,
      } = params
      const templateId = '14181'
      if (templateId === '') {
